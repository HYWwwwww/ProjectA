    ...
    <div class="modal-content">
     <!-- 모달 헤더 -->
      ...
      <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="modalclose()">닫기</button>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <?php
        		...
        		if($button=="delete"){
	        		...
	        		$writedate = $_POST['writedate'];
							...
							if(isset($_FILES['filename']) && $_FILES['filename']['name'] != "") {
							    ...
							    // 파일이름 $path
							    $filename = $path;
									$query = "insert into wp_bbs(bbstype,title,contents,id,filename,writedate) values ('notice','$title','$contents','$name','$path','$writedate' )";
									$result = $Mydb->Query($query);
        	...
        	if ($_SERVER["REQUEST_METHOD"] == "GET") { // 뷰
        		$idx = $_GET["idx"];
        		$view = $_GET["view"];
        		if($view=="modify"){
        ...
        <div class="card mb-4">
           <div class="card-body">
           	 <form id = "" method="post" action="noticewrite.php" enctype="multipart/form-data"  onsubmit="return validate()">
				    ...
				    <label for="exampleInputPassword1">제목</label>
					    <div class="input-group">
						    ...
						    <input type="hidden" class="form-control" name="bbs" value="notice">
